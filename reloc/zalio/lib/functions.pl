...
sub load_clusterHealth {
  ...
  load_json($conf{url}->{cluster_health}, $_health_ref);
...
sub load_clusterState {
  ...
  load_json($conf{url}->{cluster_state}, $_state_ref);
...
sub load_nodeStats {
  ...
  load_json($conf{url}->{node_stats}, $_stats_ref);
  foreach my $n (keys(%{$_stats_ref->{nodes}})) {
     if ( $n->{host} eq hostname() ) {
...
sub load_zalEsIndicesTable { 
  ...
  printf STDERR "DEBUG: %s: %s\n", __SUB__, ref($_indices_ref) if ( $debug );
  return($_indices_ref);
}  
# -------------------------------------------------------
...
sub get_zalEsShrdStatus { 
  ...
  # The values of the oid elements for the indexes
  my $idx_zalEsIndicesIndex = getOidElement($oid, 13);
  printf STDERR "DEBUG: %s: %s, index %s\n", __SUB__, $oid, $idx_zalEsIndicesIndex if ( $debug );
